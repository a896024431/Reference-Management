...
1. **做笔记**：仅当用户明确指定 `文献/` 内的一篇本地主文 PDF 要新建或修改笔记时，读取 `.agents/skills/DeepPaperNote/SKILL.md` 并只处理该篇论文及同目录 SI。此时才执行全文证据、临时视觉阅读、lint、第二读和发布流程；PDF 解析失败或证据不足时停止。新笔记输出固定为同目录的 `笔记.md`，发布器绝不改动 PDF。
...
- 临时同步状态、临时视觉页面和质量检查过程不得写入永久论文笔记；新笔记只保留可靠文字证据与自然叙述。
...
- `文献/**/<论文标题>/images/` 下的既有常见图片文件（不含 `Zotero已删除/`）
...
- 明确新建或修改某篇笔记：仅对该篇执行 DeepPaperNote 的正式流程和所需复核；发布时原子重建导航。全 Vault lint 仅在 CI 或用户明确要求的维护任务中运行，避免无关旧笔记阻塞单篇发布。
