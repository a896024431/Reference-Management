...
本仓库根目录是 Obsidian Vault。项目目标很窄：先用仓库内的 `zotero-pdf-sync` skill 将 Zotero 的 PDF 手动镜像到 Vault，再用 DeepPaperNote 为其中一篇本地论文生成高质量 Markdown 深读笔记；最后由用户在 Codex 侧边栏手动同步到 GitHub。
...
2. **同步 Zotero**：仅当用户主动要求同步文献库、分类或单篇论文时，读取 `.agents/skills/zotero-pdf-sync/SKILL.md`。该 skill 只从 Zotero `我的文库 / ZJU / 课题组` 的只读 Local API 镜像 PDF/SI 到 `文献/`，不读写 SQLite、不使用云端 key、不部署监听，也不读取、lint、复核或改写已有笔记。同步索引和报告只写入 `.local/zotero-pdf-sync/`；Zotero 删除附件时不得删除本地 PDF，含 `笔记.md` 的目录只能报告、不能自动移动。
...
- Zotero 是 PDF 同步入口，而不是 DeepPaperNote 的运行时元数据来源。只有 `zotero-pdf-sync` 可以调用 Local API；DeepPaperNote 的正常流程只处理已经镜像到 `文献/` 的本地文件。
...
- 手动同步 Zotero：检查同步报告和受影响文件；不读取、lint 或复核已有笔记。只有修改同步 skill 的代码或测试时，才运行该 skill 的 Ruff 和完整 pytest。
