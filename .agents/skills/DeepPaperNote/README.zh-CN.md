...
[![图表](https://img.shields.io/badge/figures-reader--clean-f59e0b)](./references/figure-placement.md)
...
> [!important] 本仓库覆盖规则（schema v2）
> 本 Vault 的正式输出固定为 `Research/<论文标题>/笔记.md` 与同目录 `images/`。
> 运行产物只进入 `.local/deeppapernote/runs/<run_id>/`；下文历史版本中的
> `Research/Papers` 和 `tmp/` 示例仅用于旧版兼容，不是发布目标。
> Zotero/infiniCloud 只是可选 provider，可用性只写入 run manifest，绝不写进永久笔记。
> 约束以 `SKILL.md`、`references/v2-workflow.md` 和 `references/vault-v2.md` 为准。
...
| 想精读一篇经典论文，但不想手写很多机械笔记 | 自动完成元数据整理、结构搭建、可追溯的图表决策和正文笔记生成，专注于真正有价值的理解 |
...
| 图表面向读者 | 每个重要图表都有可追溯决策，但笔记只展示可靠图片与自然图注。 |
...
DeepPaperNote 会复核每个重要图表，但不会把复核过程写进最终笔记。

- 可靠图片以本地 Obsidian 嵌入和简短自然图注出现；
- 没有可靠候选的图表保留在运行记录中，便于以后追溯；
- 读者不会看到候选文件、裁剪说明、放置建议、审核状态或人工占位块。

例如：

```md
![[images/fig-doc-example-p0003-fig-2.png|420]]
*Fig. 2：器件几何与测量回路。它说明了为何两个边缘通道能够被分别调节。*
```

目标是：即使某张图无法可靠发布，整篇笔记仍然连贯、可读。详见 [图表放置规则](./references/figure-placement.md)。
