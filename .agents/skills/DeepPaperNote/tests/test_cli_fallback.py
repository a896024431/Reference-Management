...
def test_write_note_falls_back_to_workspace(tmp_path: Path) -> None:
    ...
    assert payload["subdir"] == "Fallback Output Test"
    assert note_path == (tmp_path / "DeepPaperNote_output" / "Fallback Output Test" / "笔记.md")
...
def test_write_note_in_vault_mode_uses_single_note_filename(tmp_path: Path) -> None:
    ...
    payload = json.loads(result.stdout)
    note_path = Path(payload["note_path"])
    assert note_path == vault / "Research" / "心理健康" / "My_Test_Paper" / "笔记.md"
    assert note_path.exists()
